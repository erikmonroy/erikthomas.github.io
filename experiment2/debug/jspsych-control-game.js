  ...
  plugin.info = {
    ...
    parameters: {
      ...
      events: {
        ...
        default: [{type:'lo',factor:0,stretch:1,time:45},{type:'hi',factor:0,stretch:1,time:420}]
      }
    }
  }
  ...
  plugin.trial = function(display_element, trial) {
    ...
    function updateCarb(x)	{
      for (let i = 1; i < 96; i++) {
        ...
        bg_array[i] = Math.round(bg_array[i-1] + trial.carb_curve[i]*x);
        console.log(bg_array[i])
      }
    }
    
    function updateInsulin(x) {
      for (let i = 1; i < 96; i++) {
        bg_array[i] = Math.round(bg_array[i-1] + trial.insulin_curve[i]*x);
        console.log(bg_array[i])
      }
    }
